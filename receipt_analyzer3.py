...
uploaded_image = st.file_uploader("영수증 이미지를 업로드 하세요:", type=["jpg", "jpeg", "png"])
...
def extract_receipt_info(image):
  ...
  # 업스테이지의 다양한 핵심 정보 추출 모델을 활용해 여러 가지 서식의 이미지에서 정보를 추출할 수 있습니다.
  # 그중에서 한국어 영수증에서 정보를 추출해 분석할 때는 receipt-extraction 모델을 사용합니다.
  # 이렇게 모델 별칭을 지정하면, 자동으로 최신 모델이 사용됩니다.
...
def classify_expense(exponse_details):
    ...
    if 'fields' in receipt_data:
      ...
      for field in receipt_data.get('fields', []):
        ...
        # 카드 번호
          ...
          card_nember = value
        elif key == 'transcation.cc_number' and field_type == 'content':
          card_number = value
      ...
      output += f"\n거래일시: {date if date else '찾을 수 없음'}" #<-⚠️
      ...
      output += f"\n합계 금액: {total_amount if total_amount else '찾을 수 없음'}" #<-⚠️
      output += f"\n결제액: {payment_price if payment_price else '찾을 수 없음'}" #<-⚠️
      output += f"\n지불 수단: {payment_method if payment_method else '찾을 수 없음'}" #<-⚠️
      output += f"\n카드 번호: {card_number if card_number else '찾을 수 없음'}" #<-⚠️
      output += f"\n지출 분류: {expense_category}" #<-⚠️

      st.code(output, language='plain')
    else:
      st.error("영수증 정보를 추출할 수 없습이다. 다른 영수증 이미지를 시도해 주세요")
