...
if uploaded_file is not None:
  ...
  if format_choice == "conversational single-turn chat":
    default_prompt = "Marv is a factual chatbox that is also sarcastic."
    system_prompt = st.text_input("System Prompt", default_prompt)
  if st.button("Convert to JSONL"):
    st.session_state.jsonl_str = convert_to_jsonl(st.session_state.df, format_choice,
...
if st.session_state.get('jsonl_str'):
  ...
  st.download_button(label="Download JSONL", data=st.session_state.jsonl_str.encode('utf-8'), file_name=download_filename, mime='text/plain')
